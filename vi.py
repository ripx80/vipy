...
import logging
#gentoo useflags: media-video/ffmpeg threads zlib encode x264 hardcoded-tables truetype fdk libass
...
class Extractor(object):	
	def __init__(self,commands,movfile,raw,preview,copy,force,encaudio):		
	...
	def setCrop(self):
		...
		crop=check_output(self.commands['ffmpeg']+' -y -i "%s" -vf cropdetect -t 100 -ss 600 -an -f rawvideo /dev/null 2>&1 | grep crop | tail -n1 | cut -f15 -d\ '%(self.movfile,),shell=True).replace("\n", "")
		if len(crop)<=0:
	...
	def scanStreams(self):
		...
		for i in check_output(self.commands['ffmpeg']+' -i "%s" 2>&1 | grep "Stream" | sed -e \'s/^[ \t]*//\''%(self.movfile,),shell=True).split('\n'):			
			...
			try:
				...
				log.error('canot find map in line: '+i)		
				exit(1)				
		self.streams=tmp	
	...
	def extract_m4v(self):
		...
		call(self.commands['ffmpeg']+' -y -sn -an -i "%s" -map %s -vf %s %s "%s/main.m4v"'%(self.movfile,self.streams['Video'][0][0],self.crop,codec,self.raw),shell=True);
	...
	def extract_srt(self):
		for i in self.streams['Subtitle']:
			...
			#print self.commands['ffmpeg']+' -i "%s" -scodec srt -map %s -an -vn -y "%s/%s.srt"'%(self.movfile,i[0],self.raw,i[1])
...
if args.loopdir:
	...
	for i in dirs:		
		if args.extract:			
			ext=Extractor(commands,'%s/%s'%(args.loopdir,i),'%s/raw'%(args.root,),args.preview,args.copy,args.force,args.encaudio)
			packraw=ext.extract()
	...
	if args.extract:	
		ext=Extractor(commands,args.single,'%s/raw'%(args.root,),args.preview,args.copy,args.force,args.encaudio)
		packraw=ext.extract()
